...
setGeneric("FLFcube_AD", function(fleets,stocks,control,...){
        standardGeneric("FLFcube_AD")
})

setMethod("FLFcube_AD", signature(fleets="FLFleets",stocks="FLStocks",control="FLFcube.control"),
...
if(length(grep("rev",control@effort.rule,ignore.case=T))>0 | length(grep("val",control@effort.rule,ignore.case=T))>0) {
    ...
    for (S in sp.lst) {
              print(S)
               ...
               age.q <- as.character(unique(res.$age))
               if (all(!age.q=="all")) {                                   
               res. <- res.[res.$age %in% c(range[S,"minfbar"]:range[S,"maxfbar"]),]
               res. <- aggregate(list(land.q=res.$land.q),list(year=res.$year,unit=res.$unit,season=res.$season, area=res.$area,
                                    iter=res.$iter,spp=res.$spp,fleet=res.$fleet,metier=res.$metier),mean,na.rm=T)} else {
                                     res. <- res.[,c("year","unit","season", "area","iter","spp","fleet","metier","land.q")] 
                                    }

              # names(qq.)<-gsub("x","land.q",names(qq.))
